/*=================================================
  公共复用模块
	1、GetFileName()		获取文件名称
	2、ChineseConvertPy()   将中文汉字转换成拼音
	3、GetFirstLetter()     获取中文汉字的拼音首字母
	4、ColourPrintf()       高亮字体 
  author :  zhangxu 
===================================================*/
...
#ifdef __TRACE__ //输出调用函数的信息

	fprintf(stdout, "[TRACE][%s:%d:%s]:", GetFileName(filename).c_str(), line, function);

	//输出用户打的trace信息 va_list args; va_start(args,format); vfprintf(stdout,format, args); va_end(args);
